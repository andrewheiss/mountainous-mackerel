...
f_h3 <- function(panel) {
  BAYES_SEED <- 8412  # From random.org
  ...
  h3_models <- tribble(
    ~nice, ~y, ~never,
    "Income Support", "e1_income_support_bin", "e1_income_support_never",
    "Debt Relief", "e2_debt_relief_bin", "e2_debt_relief_never"
  ) %>% 
    mutate(data = map(never, ~never_filter(.))) %>% 
    mutate(model = map2(y, data, ~h3_model(.x, .y)))
